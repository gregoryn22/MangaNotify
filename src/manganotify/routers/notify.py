...
async def notify_test(request: Request, current_user: dict = Depends(require_auth)):
    ...
    if not (settings.PUSHOVER_APP_TOKEN and settings.PUSHOVER_USER_KEY):
        return JSONResponse(
            status_code=500,
            content={"ok": False, "message": "Missing Pushover env vars"},
        )
    res = await pushover(
        request.app.state.client, "MangaNotify", "✅ test", settings_obj=settings
    )
    add_notification(
        "test",
        {
...
async def set_discord_settings(
    body: DiscordSettings, request: Request, current_user: dict = Depends(require_auth)
):
    # Get settings from app state
    settings = request.app.state.settings
...
async def discord_test(request: Request, current_user: dict = Depends(require_auth)):
    ...
    result = await discord_notify(
        client, "MangaNotify Test", "This is a test notification from MangaNotify."
    )
