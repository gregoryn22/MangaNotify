...
from ..core.utils import str_eq, to_bool_or_none
...
async def search(
    ...
    try:
        ...
        if (
            content_rating
            and not content_rating.replace("_", "").replace("-", "").isalnum()
        ):
        ...
        items = [
            normalize_series_min(it)
            for it in (raw.get("data") or raw.get("results") or [])
        ]
