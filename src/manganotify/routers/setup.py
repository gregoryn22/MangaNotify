...
from ..core.crypto import encrypt_credential, generate_master_key
...
class SetupRequest(BaseModel):
    ...
    username: str = Field(
        ...
        description="Admin username (alphanumeric, underscore, hyphen only)",
    )
    password: str = Field(
        ...,
        min_length=8,
        max_length=128,
        description="Admin password (min 8 chars, max 128)",
    )

    # Notification credentials (optional)
    pushover_app_token: str | None = Field(
        default=None, max_length=100, description="Pushover app token"
    )
    pushover_user_key: str | None = Field(
        default=None, max_length=100, description="Pushover user key"
    )
    ...
    master_key: str | None = Field(
        default=None, min_length=32, max_length=100, description="Master encryption key"
    )
...
async def configure_setup(
    request: SetupRequest, current_user: dict = Depends(require_auth)
) -> SetupResponse:
    ...
    if request.auth_enabled:
        ...
        if request.username.lower() in sensitive_usernames:
            logger.warning(
                "User chose potentially sensitive username: %s", request.username
            )
    ...
    if request.discord_webhook_url and not request.discord_webhook_url.startswith(
    ...
    if (
        request.master_key
        and not request.master_key.replace("_", "").replace("-", "").isalnum()
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if request.pushover_app_token:
        env_config["PUSHOVER_APP_TOKEN"] = encrypt_credential(
            request.pushover_app_token, master_key
        )
    if request.pushover_user_key:
        env_config["PUSHOVER_USER_KEY"] = encrypt_credential(
            request.pushover_user_key, master_key
        )
    if request.discord_webhook_url:
        env_config["DISCORD_WEBHOOK_URL"] = encrypt_credential(
            request.discord_webhook_url, master_key
        )
        env_config["DISCORD_ENABLED"] = "true"

    # Add other recommended settings
    env_config["CORS_ALLOW_ORIGINS"] = (
        "https://yourdomain.com"  # User MUST change this in production
    )
