...
def _should_send_notification(series_item: dict) -> bool:
    ...
    if notif_prefs.get("only_when_reading", True):
        ...
        if series_status not in [
            "reading",
            "releasing",
        ]:  # Allow both reading and releasing statuses
            return False
    ...
    for it in wl:
        ...
        try:
            ...
            if attempts >= 3 and last_exc is not None:
                logging.warning(
                    "poller: failed to fetch series %s after retries: %s", sid, last_exc
                )
                continue
            ...
            if (
                ...
                if should_notify:
                    ...
                    if discord_enabled:
                        ...
                        discord_result = await discord_notify(
                            client, "New chapter(s)", msg
                        )

                    # Log notification results
                    push_ok = (
                        bool(pushover_result and pushover_result.get("ok"))
                        if pushover_result
                        else False
                    )
                    discord_ok = (
                        bool(discord_result and discord_result.get("ok"))
                        if discord_result
                        else False
                    )
            ...
            if new_total is not None:
                it["total_chapters"] = new_total
            if c := pick_cover(series):
                it["cover"] = c
