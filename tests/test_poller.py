...
from manganotify.services.poller import (
...
class TestPollerLogic:
    ...
    def test_should_send_notification_defaults(self):
        ...
        series_item = {"id": 1677, "title": "Chainsaw Man", "status": "reading"}

        assert _should_send_notification(series_item)

        # Series with notifications disabled
        series_item["notifications"] = {"enabled": False}
        assert not _should_send_notification(series_item)
...
class TestPollerIntegration:
    ...
    async def test_poller_detects_new_chapter(self, temp_watchlist, temp_data_dir):
        ...
        with patch(
            ...
            with patch(
                "manganotify.services.poller.pushover", new_callable=AsyncMock
            ) as mock_pushover:
                ...
                with (
                    patch(
                        "manganotify.services.poller.load_watchlist",
                        side_effect=mock_load_watchlist,
                    ),
                    patch(
                        "manganotify.services.poller.save_watchlist",
                        side_effect=mock_save_watchlist,
                    ),
                ):
                    # Run the poller once
            ...
            assert updated_watchlist is not None, (
                "save_watchlist should have been called"
            )
            test_series_items = [
                item for item in updated_watchlist if item["id"] == 1677
            ]
            assert len(test_series_items) > 0, (
                f"No test series found in watchlist: {updated_watchlist}"
            )
...
class TestNotificationContent:
    ...
    def test_notification_message_formatting(self):
        ...
        expected_message = (
            f"{series_title} now has {new_total} chapters. You're {unread} behind."
        )
    ...
    async def test_missed_update_scenario(self, temp_data_dir):
        ...
        with patch(
            ...
            chapter_notifications = [
                n for n in notifications if n.get("kind") == "chapter_update"
            ]
            ...
            assert latest_notification["notifications_enabled"]
